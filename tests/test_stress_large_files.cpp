...
std::filesystem::path output_path_for_input(const std::filesystem::path& input_path) {
...
TEST(StressLargeFiles, ProcessBigFileSoftThroughput) {
    if (!env_flag_enabled("FLOWER_EXCHANGE_RUN_STRESS")) {
        GTEST_SKIP() << "Set FLOWER_EXCHANGE_RUN_STRESS=1 to execute large-file stress runs.";
    }

    const auto big_path = repo_path("big_orders.csv");
    ASSERT_TRUE(std::filesystem::exists(big_path));

    const double budget_sec = env_double("FLOWER_EXCHANGE_STRESS_BIG_MAX_SECONDS", 900.0);
...
TEST(StressLargeFiles, BigFileByteDeterministicAcrossFiveRuns) {
    if (!env_flag_enabled("FLOWER_EXCHANGE_RUN_STRESS_DETERMINISM")) {
        GTEST_SKIP() << "Set FLOWER_EXCHANGE_RUN_STRESS_DETERMINISM=1 to run 5-run byte checks.";
    }
