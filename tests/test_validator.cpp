...
TEST(ValidatorTest, PriceBoundaries) {
  ...
  EXPECT_FALSE(
      validate_order("Rose", 1, MAX_VALID_PRICE + TICK_SIZE, 100).first);
}

TEST(ValidatorTest, RejectsNonFinitePrices) {
  EXPECT_FALSE(
      validate_order("Rose", 1, std::numeric_limits<double>::infinity(), 100)
          .first);
  EXPECT_FALSE(
      validate_order("Rose", 1, -std::numeric_limits<double>::infinity(), 100)
          .first);
  EXPECT_FALSE(
      validate_order("Rose", 1, std::numeric_limits<double>::quiet_NaN(), 100)
          .first);
}
