...
int run_with_commands(const std::vector<std::string>& commands) {
    ...
    shell << "cat \"" << cmd_file.string() << "\" | \""
          << FLOWER_EXCHANGE_BIN << "\" >/dev/null 2>&1";
...
int count_char(const std::string& text, char ch) {
    ...
    std::count_if(text.begin(), text.end(), [ch](char c) -> bool { return c == ch; }));
}
...
TEST(ApplicationIntegrationTest, RepeatedProcessResetsOrderSequence) {
    ...
    const auto lines = read_lines(find_output_file("example3_orders_reports.csv"));
...
TEST(ApplicationIntegrationTest, MultipleSequentialRunsRemainDeterministic) {
    ...
    for (int i = 0; i < 50; ++i) {
        commands.push_back(
            "PROCESS " + repo_path("input_files/example5_orders.csv").string());
    }
    ...
    const auto lines = read_lines(find_output_file("example5_orders_reports.csv"));
...
TEST(ApplicationIntegrationTest, ExampleFixturesMatchCanonicalOutput) {
    ...
    for (int i = 1; i <= 7; ++i) {
        ...
        ASSERT_EQ(actual_lines.size(), expected_rows[static_cast<std::size_t>(i - 1)].size())
        ...
        for (std::size_t row = 0; row < actual_lines.size(); ++row) {
            EXPECT_EQ(drop_last_csv_column(actual_lines[row]),
                      drop_last_csv_column(expected_rows[static_cast<std::size_t>(i - 1)][row]))
...
TEST(ApplicationIntegrationTest, RepeatedRunOutputIsByteStable) {
    ...
    const std::string first_output = read_file_text(find_output_file(output_name));
    ...
    const std::string second_output = read_file_text(find_output_file(output_name));
...
TEST(ApplicationIntegrationTest, RejectedOrdersProduceExactlyOneRejectRowEach) {
    const std::filesystem::path input_path = repo_path("input_files/reject_reason_matrix_orders.csv");
    {
    ...
    const auto lines = read_lines(find_output_file("reject_reason_matrix_orders_reports.csv"));
...
TEST(ApplicationIntegrationTest, OrderIdsProgressAfterRejection) {
    const std::filesystem::path input_path = repo_path("input_files/reject_then_valid_orders.csv");
    {
        std::ofstream out(input_path);
        out << "Client Order ID,Instrument,Side,Quantity,Price\n";
        out << "x1,,1,100,55.00\n";
    ...
    const auto lines = read_lines(find_output_file("reject_then_valid_orders_reports.csv"));
    ASSERT_EQ(lines.size(), 5u);
...
TEST(ApplicationIntegrationTest, OversizedAndNonFinitePricesAreRejectedWithoutCrash) {
    const std::filesystem::path input_path = repo_path("input_files/price_overflow_orders.csv");
    {
        std::ofstream out(input_path);
        out << "Client Order ID,Instrument,Side,Quantity,Price\n";
    ...
    const auto lines = read_lines(find_output_file("price_overflow_orders_reports.csv"));
