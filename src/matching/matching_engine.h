...
public:
    ...
    MatchingEngine(MemPool<Order>& order_pool,
    ...
    void process_order(Order* incoming, EmitReport&& emit_report) {
        ...
        while (incoming->quantity > 0) {
            int64_t best_level =
                (incoming->side == Side::BUY) ? opp_book.find_first() : opp_book.find_last();
            if (best_level < 0) {
                break;
            }
            ...
            const uint16_t trade_qty = std::min(incoming->quantity, resting->quantity);
            incoming->quantity = static_cast<uint16_t>(incoming->quantity - trade_qty);
            ...
            emit_report(report_pool_.allocate(resting->oid,
                                              resting->coid,
                                              resting->instrument,
                                              resting->side,
                                              best_price,
