...
constexpr std::size_t ORDER_BOOK_TICK_CAPACITY =
    RuntimeConfig::kOrderBookTickCapacity;
constexpr PriceTick MAX_VALID_TICK =
    static_cast<PriceTick>(RuntimeConfig::kMaxValidTick);
...
struct ExecutionReport {
  ...
  static inline void fill_current_timestamp(char *out, std::size_t out_size) {
    ...
    const int written = std::snprintf(
        out, out_size, "%04d%02d%02d-%02d%02d%02d.%03d",
        tm_local.tm_year + 1900, tm_local.tm_mon + 1, tm_local.tm_mday,
        tm_local.tm_hour, tm_local.tm_min, tm_local.tm_sec, millis);
  ...
  ExecutionReport()
      : oid{0}, coid{0}, instrument(InstrumentType::COUNT), side(Side::BUY),
        price(0), quantity(0), status(ExecStatus::NEW), reason{0},
        timestamp{0} {
