...
int string_view_to_int(std::string_view sv) {
    ...
    if (i >= sv.size()) throw std::invalid_argument("stoi");
...
double string_view_to_double(std::string_view sv) {
    ...
    // Skip leading whitespace
    while (i < sv.size() && (sv[i] == ' ' || sv[i] == '\t')) ++i;
    if (i >= sv.size()) throw std::invalid_argument("stod");
...
*
* Returns the number of characters written excluding the null terminator, or 0 on error.
*/
std::size_t format_ticks_to_buffer(PriceTick ticks, char* buffer, std::size_t buffer_size) {
    ...
    const auto integer_part =
        static_cast<unsigned long long>(ticks / 100ULL);
...
std::size_t format_price_to_buffer(double price, char* buffer, std::size_t buffer_size) {
    ...
    if (price >= 0.0) {
        const std::size_t len = format_ticks_to_buffer(double_to_ticks(price), buffer, buffer_size);
        if (len > 0) {
            return len;
        }
    }
